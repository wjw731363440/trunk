...
YADE_PLUGIN((Ip2_FrictMat_FrictMat_LubricationPhys)(LubricationPhys)(Law2_ScGeom_ImplicitLubricationPhys)(LubricationPDFEngine)(LPDFEngine))
...
{
	Real a((geom->radius1+geom->radius2)/2.);
	if(resolution != 1 && resolution != 3 && debug && !warnedOnce) { LOG_DEBUG("This method use log(u/a) for shear and torque component calculation. Make sure phys->delta is set before calling this method."); warnedOnce = true; }
...
// 			//LOG_DEBUG(" vars " << geom->normal << " " << r << " " << theta << " " << phi << " " << dS << " " << l);
// 			
...
template<class Phys>
void PDFEngine<Phys>::getStressSpectrums(vector<vector<vector<Matrix3r> > > &spects, vector<DataVector> const& origins, int nTheta, int nPhi)
{
	const shared_ptr<Scene>& scene=Omega::instance().getScene();
	...
	FOREACH(const shared_ptr<Interaction>& I, *scene->interactions) {
		...
		if(phys && geom)
		{
			...
			for(uint j(0);j<origins.size();j++) {
				spects[j][idT1][idP1] += phys->*(origins[j].member)*l.transpose();
				spects[j][idT2][idP2] += phys->*(origins[j].member)*l.transpose();
			}
		}
	}
}
...
template<class Phys>
void PDFEngine<Phys>::writeToFile(vector<vector<vector<Matrix3r> > > const& stress_data, vector<DataVector > const& stress_meta,
		vector<vector<vector<Matrix3r> > > const& mat_data, vector<DataMatrix > const& mat_meta,
		vector<vector<vector<Vector3r> > > const& vector_data, vector<DataVector > const& vector_meta,
		vector<vector<vector<Real> > > const& scalar_data, vector<DataScalar > const& scalar_meta)
{
		...
		if(firstRun) {
			...
			for(uint l(0);l<stress_meta.size();l++)
				for(int i(0);i<numDiscretizeAngleTheta;i++)
					for(int j(0);j<numDiscretizeAnglePhi;j++)
						for(int k(0);k<9;k++)
							fprintf(fid,"%s_%s(%f,%f)\t", stress_meta[l].name.c_str(), mat_suffix[k].c_str(), i*Mathr::PI/numDiscretizeAngleTheta, j*Mathr::PI*2./numDiscretizeAnglePhi - Mathr::PI);
			...
			for(uint l(0);l<mat_meta.size();l++)
				for(int i(0);i<numDiscretizeAngleTheta;i++)
					for(int j(0);j<numDiscretizeAnglePhi;j++)
						for(int k(0);k<9;k++)
							fprintf(fid,"%s_%s(%f,%f)\t", mat_meta[l].name.c_str(), mat_suffix[k].c_str(), i*Mathr::PI/numDiscretizeAngleTheta, j*Mathr::PI*2./numDiscretizeAnglePhi - Mathr::PI);
						
			// Vector
			for(uint l(0);l<vector_meta.size();l++)
				for(int i(0);i<numDiscretizeAngleTheta;i++)
					for(int j(0);j<numDiscretizeAnglePhi;j++)
						for(int k(0);k<3;k++)
							fprintf(fid,"%s_%s(%f,%f)\t", vector_meta[l].name.c_str(), vec_suffix[k].c_str(), i*Mathr::PI/numDiscretizeAngleTheta, j*Mathr::PI*2./numDiscretizeAnglePhi - Mathr::PI);
			
			// Scalar
			for(uint l(0);l<scalar_meta.size();l++)
				for(int i(0);i<numDiscretizeAngleTheta;i++)
					for(int j(0);j<numDiscretizeAnglePhi;j++)
						fprintf(fid,"%s(%f,%f)\t", scalar_meta[l].name.c_str(), i*Mathr::PI/numDiscretizeAngleTheta, j*Mathr::PI*2./numDiscretizeAnglePhi - Mathr::PI);
			fprintf(fid, "\n");
			firstRun = false;
		}
...
{
	vector<DataVector> origin;
	DataVector o;
	
	o.member = &LubricationPhys::normalContactForce;
	o.name = "NC";
	
	origin.push_back(o);
	
	o.member = &LubricationPhys::shearContactForce;
	o.name = "SC";
	
	origin.push_back(o);
	
	o.member = &LubricationPhys::normalLubricationForce;
	o.name = "NL";
	
	origin.push_back(o);
	
	o.member = &LubricationPhys::shearLubricationForce;
	o.name = "SL";
	
	origin.push_back(o);
	
	vector<vector<vector<Matrix3r> > > stresses;
	getStressSpectrums(stresses, origin, numDiscretizeAngleTheta, numDiscretizeAnglePhi);
	
	vector<vector<vector<Matrix3r> > > mat_data;
	vector<DataMatrix> mat_meta;
	vector<vector<vector<Vector3r> > > vector_data;
	vector<DataVector> vector_meta;
	vector<vector<vector<Real> > > scalar_data;
	vector<DataScalar> scalar_meta;
	
	writeToFile(stresses, origin, mat_data, mat_meta, vector_data, vector_meta, scalar_data, scalar_meta);
}

CREATE_LOGGER(LubricationPDFEngine);
CREATE_LOGGER(PDFEngine<LubricationPhys>);
